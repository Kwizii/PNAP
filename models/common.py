...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        cuda_ = torch.cuda.is_available() and device.type != "cpu"  # use CUDA
        if not (pt or triton):
        ...
        if pt:  # PyTorch
            ...
            check_requirements(("onnx", "onnxruntime-gpu" if cuda_ else "onnxruntime"))
            import onnxruntime

            providers = ["CUDAExecutionProvider", "CPUExecutionProvider"] if cuda_ else ["CPUExecutionProvider"]
            ...
            check_requirements("paddlepaddle-gpu" if cuda_ else "paddlepaddle")
