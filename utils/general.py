...
def non_max_suppression_gradable(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False,
                                 multi_label=False,
                                 labels=(), max_det=300):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x1 = x.clone()
        obj_conf = x1[:, 4:5]
        x[:, 5:] *= x1[:, 5:] * obj_conf  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            x = torch.cat((box, obj_conf, conf, j.float()), 1)[conf.view(-1) > conf_thres]
